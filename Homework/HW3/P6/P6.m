...
% class1_std = 1;
% class2_std = 1.1;
% X1 = [class1_std*randn(N,2)+mu1;2*class2_std*randn(N,2)+mu2];
...
for ii=1:size(ws,1)
  ...
  set(h, 'linestyle', '-', 'linecolor', colors(ii), 'linewidth', 2);
end
...
title('Probability Level Sets of Log-Prior', 'fontsize', fs);
...
title('Probability Level Sets of Log-Unnormalised Posterior', 'fontsize', fs)
hold on
...
hold on
plot(W(j,1),W(j,2),'.','MarkerSize',40);
title('Probability Level Sets of Laplace Approximation to Posterior', 'fontsize', fs)
grid on
